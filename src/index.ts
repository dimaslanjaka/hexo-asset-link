...
if (hexo.config.post_asset_folder === true) {
  hexo.extend.filter.register('before_post_render', async function (data) {
    if (!data.asset_dir) {
      ...
      data.asset_dir = data.source.replace(hexo.config.source_dir + '/_posts/', '').replace(/.(md)$/, '');
    }
    ...
    const path_markdown = RegExp(
      look_behind + '(./)?' + asset_dir_name.replace(/[/\-\\^$*+?.()|[\]{}]/g, '\\$&') + '/',
      'g'
    );
    ...
    hexo.log.i('Path converted:', ansiColors.yellow(path_markdown.toString()), '→', ansiColors.green(path_html));
  });
}
